...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # adjust if needed for specific frontend domain
...
def fetch_dune_whale_data():
    ...
    try:
        ...
        if "error" in run_data and "datapoint limit" in run_data["error"].lower():
            print("⚠️ Dune datapoint limit reached. Using cached whale data if available.")
            return whale_cache["data"] if whale_cache["data"] is not None else pd.DataFrame()
